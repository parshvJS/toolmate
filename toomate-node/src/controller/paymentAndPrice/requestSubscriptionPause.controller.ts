...
export async function requestSubscriptionPause(req: Request, res: Response) {
  try {
    const { subscriptionId, userId, message, isDownGradeRequest, downGradeDuration } = req.body;
    ...
    if (message !== "downgrade" && message !== "suspend" && message !== "cancel") {
      return res.status(400).json({
        success: false,
        status: 400,
        message: "Invalid message type. Please provide downgrade, suspend, or cancel"
      });
    }
    const validDownDate = [1, 6, 12]
    if (isDownGradeRequest && !downGradeDuration && !validDownDate.includes(downGradeDuration)) {
      return res.status(400).json({
        success: false,
        status: 400,
    ...
    const nextBillingDateString = subscriptionDetails.data.billing_info.next_billing_time;
    ...
    return res.status(200).json({
      success: true,
      status: 200,
      message: "Request to suspend subscription has been saved to the queue"
    });
  } catch (error: any) {
