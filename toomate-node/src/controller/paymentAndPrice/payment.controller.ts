...
async function createBillingPlan(
	...
	if (firstBillingCycleOnly) {
		...
		console.log(
			'Discounted price for first billing cycle:',
			discountedPrice
		);
...
export async function Payment(req: Request, res: Response) {
	...
	try {
		...
		if (
			!paymentData?.essentialProductId?.length ||
			!paymentData.proProductId?.length
		) {
		...
		if (isCouponCodeApplied) {
			if (!couponInput) {
				return res
					.status(400)
					.json({ message: 'Coupon Code is required' });
			}
			...
			if (!coupon) {
				return res
					.status(404)
					.json({ message: 'Coupon Code not found' });
			}
			...
			if (!newBillingPlan) {
				return res
					.status(404)
					.json({ message: 'No billing plan found' });
			}
		...
		if (!subscription) {
			return res
				.status(404)
				.json({ message: 'No subscription data found' });
		}

		if (subscription.status === 401 || subscription.data.status === 401) {
			return res
				.status(401)
				.json({ message: 'Unauthorized', data: subscription.data });
		}

		const approvalLink = subscription.data.links?.find(
			(link: any) => link.rel === 'approve'
		);
		const baToken = approvalLink
			? new URL(approvalLink.href).searchParams.get('ba_token')
			: null;
		...
		return res
			.status(200)
			.json({ url: approvalLink.href, message: 'Redirect to PayPal' });
	} catch (error: any) {
		console.log(error.message);
		return res.status(500).json({ message: 'Internal Server Error' });
	}
